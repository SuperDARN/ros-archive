...
int SiteFhwSetupRadar() {
  ...
  strcpy(sharedmemory,"IQBufferfhw");
...
int SiteFhwIntegrate(int (*lags)[2]) {
  ...
  badrng=ACFBadLagZero(&tsgprm,mplgs,lags);
  ...
  while (1) {
    ...
    memcpy(samples+iqoff,rdata.back,rdata.samples*4);
    ...
    aflg=ACFSumPower(&tsgprm,mplgs,lags,pwr0,
    ...
    ACFCalculate(&tsgprm,(int16 *) rdata.main,2,1,
	  	 ...
	  	 lags,acfd,ACF_PART,2,badrng,atten*atstp,NULL);
