...
/*
 $Log: fitwrite.c,v $
 Revision 1.8  2004/08/04 22:49:54  barnes
 Fixed bug that could cause a floating point errors.

 Revision 1.7  2004/07/22 17:43:52  barnes
 Fixed rounding bug in converting from double to integer.

 Revision 1.6  2004/07/20 00:52:50  barnes
 Zeroed parameter block.

 Revision 1.5  2004/05/04 18:32:13  barnes
 Renamed the type header to avoid confusion.

 Revision 1.4  2004/04/30 22:11:20  barnes
 Changed macro definition.

 Revision 1.3  2004/04/28 18:26:42  barnes
 Fixed time_t.

 Revision 1.2  2004/04/26 22:23:36  barnes
 Audit - Enforced warning "all".

 Revision 1.1  2004/01/15 23:23:30  barnes
 Initial revision
...
int OldFitHeaderWrite(int fitfp, char *text, char *name, char *version) {
  ...
  ConvertFromShort(FIT_RECL,head_buf);
  ConvertFromShort(INX_RECL,head_buf+sizeof(int16));
...
int OldFitWrite(int fitfp,struct RadarParm *prm,struct FitData *fit,
  ...
  for (c=0;c<prm->mplgs;c++) {
    oldfit.lag[0][c]=prm->lag[c][0];
    oldfit.lag[1][c]=prm->lag[c][1];
  } 
