...
int test_gtable_write(struct gtable *ptr,struct radardata *scan,int tlen) {
  ...
  if ((ptr->ed_time-ptr->st_time) >=tlen) {
    ...
    for (i=0;i<ptr->pnum;i++) {
      ...
      ptr->pnt[i].vel.median=ptr->pnt[i].vel.median/ptr->pnt[i].cnt;
      ptr->pnt[i].wdt.median=ptr->pnt[i].wdt.median/ptr->pnt[i].cnt;
      ptr->pnt[i].pwr.median=ptr->pnt[i].pwr.median/ptr->pnt[i].cnt;
      
     
      ptr->pnt[i].vel.sd=sqrt(ptr->pnt[i].vel.sd/ptr->pnt[i].cnt);
      ptr->pnt[i].wdt.sd=sqrt(ptr->pnt[i].wdt.sd/ptr->pnt[i].cnt);
      ptr->pnt[i].pwr.sd=sqrt(ptr->pnt[i].pwr.sd/ptr->pnt[i].cnt);
    ...
    for (k=0;k<scan->bmcnt[i];k++) {
      ...
      for (j=0;j<MAX_RANGE;j++) {
        if (scan->sct[i][j][k] !=0) {
          ...
          ptr->pnt[l].vel.median+=-scan->rng[i][j][k].v;
          ptr->pnt[l].pwr.median+=scan->rng[i][j][k].p_l;
          ptr->pnt[l].wdt.median+=scan->rng[i][j][k].w_l;

          ptr->pnt[l].vel.sd+=
	    scan->rng[i][j][k].v_e*scan->rng[i][j][k].v_e;
          ptr->pnt[l].pwr.sd+=
	    scan->rng[i][j][k].p_l_e*scan->rng[i][j][k].p_l_e;
          ptr->pnt[l].wdt.sd+=
	    scan->rng[i][j][k].w_l_e*scan->rng[i][j][k].w_l_e;
