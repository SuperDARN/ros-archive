/*   badlags_s.c
     ===========
     Author: R.J.Barnes & Nigel Wade, Original code Kile Baker
*/
...
void badlags_s(struct fit_prm *ptr, struct badsmp *bptr) {
    int i, k, l, n, sample;
    long ts, t1, t2;
    int nbad;
    ...
    if (ptr->offset != 0 ) {   
       /* stereo data will now contain the offset in this variable */    
	int cp;
	cp = ptr->cp;
	if (cp< 0 ) cp = -cp;
	offset = ptr->offset;
	if (cp < 20000 ) offset = -offset;
    } else {
	int j;
	
	offset = 0;

	/* need some code here to set offset from the CPID and offset 
           stored in user variables */
	for ( j = 0; j < sizeof offsets / sizeof (offset_table); j++ ) 
	    if ( ptr->cp == offsets[j].cp ) {
    ...
    /* if offset is zero (i.e. not matched in the known CP for stereo)
        ignore the loop */
