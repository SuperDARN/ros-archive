...
double elev_goose(struct fit_prm *prm, double range, double phi0) {
  ...
  double dchi_sep_max, dchi_max, dchi, dchi_old, cs_alpha, sn2_eps, sn_eps;
  double elev;
  ...
  */
  ...
  if (sep == 0.0) {
    xi = atan2(prm->interfer_pos[0],(prm->interfer_pos[1]));
    sep = sqrt(prm->interfer_pos[0]*prm->interfer_pos[0] + 
			prm->interfer_pos[1]*prm->interfer_pos[1] +
	       prm->interfer_pos[2]*prm->interfer_pos[2]);

    elev_corr = atan2(prm->interfer_pos[2], 
			sqrt(prm->interfer_pos[0]*prm->interfer_pos[0] +
			prm->interfer_pos[1]*prm->interfer_pos[1]));
    cos_xi = cos(xi);
    sin_xi = sin(xi);
  ...
  psi = prm->beam_sep*(prm->bmnum-7.5)*PI/180.;	
