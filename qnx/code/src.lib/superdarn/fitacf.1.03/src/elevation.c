...
double elevation(struct fit_prm *prm,double range,double phi0) {
  ...
  if (antenna_separation == 0.0) {
    antenna_separation= sqrt(prm->interfer_pos[1]*prm->interfer_pos[1] + 
			                 prm->interfer_pos[0]*prm->interfer_pos[0] +
	                         prm->interfer_pos[2]*prm->interfer_pos[2]);
    elev_corr= prm->phidiff* asin( prm->interfer_pos[2]/ antenna_separation);
    if (prm->interfer_pos[1] > 0.0) /* interferometer in front of main antenna */
      phi_sign= 1.0;
  ...
  phi= prm->beam_sep*(prm->bmnum - 7.5)* PI/ 180.0;
  c_phi= cos( phi);
