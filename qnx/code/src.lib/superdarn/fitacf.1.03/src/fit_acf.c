...
int fit_acf (struct complex *acf,int range,
             int *badlag,struct badsmp *badsmp,int lag_lim,
			 struct fit_prm *prm,
		     double noise_lev_in,char xflag,double xomega,
		     struct range_data *ptr) {
      
  double sum_np,sum_w,sum_wk,sum_wk2,sum_wk2_arr[LAG_TAB_LEN],sum_wk4;
  double sum_p,sum_pk,sum_pk2,sum_phi,sum_kphi;
  double t0,t2,t4,phi_res[LAG_TAB_LEN],phi_tot,phi_pred;
  int i,j,Ist,n_twopi,icnt,lag1,lag2;
  long k;
  int nphi, npp;
  double tau[LAG_TAB_LEN], tau2[LAG_TAB_LEN];
  short badflag_1,badflag_2;
  double phi_k[LAG_TAB_LEN]; 
  double fluct, omega_old, omega_old_2;
  double w[LAG_TAB_LEN], pwr[LAG_TAB_LEN];
  double wt[LAG_TAB_LEN], wt2[LAG_TAB_LEN], wp[LAG_TAB_LEN];
  double e2;

  double c_log,c_log_err,d,db,l_max;
  struct complex z;
...
*/
 ...
 /* initialize the sums */
  ...
  calc_phi_res(acf, badlag, phi_res, prm->mplgs);
