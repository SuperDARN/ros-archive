...
int CSDReadBlock(struct CSDfp *fptr,struct CSDdata *ptr) {
  ...
  if (fptr->fp !=-1) {
    ...
    if (ConvertReadShort(fptr->fp,&ptr->intt.us) !=0) return -1;
    ...
    if ((fptr->fptr+sizeof(double)+9*sizeof(int16)+
         sizeof(int32))>fptr->fsze) return -1;
    ...
    fptr->fptr+=sizeof(int16);
    ConvertToShort(fptr->fbuf+fptr->fptr,&ptr->intt.us);
    fptr->fptr+=sizeof(int16);
  ...
  if (fptr->fp !=-1) {
    ...
    if (read(fptr->fp,&bnc,1) !=1) return -1;
    ...
    fptr->fptr+=1;
    bnc=fptr->fbuf[fptr->fptr];
    fptr->fptr+=1;
  }

  ptr->bmnum=bnc & 0x0f; 
  ptr->channel=(bnc>>4) & 0x0f;
