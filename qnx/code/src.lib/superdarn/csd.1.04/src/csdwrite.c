...
int CSDWrite(int fp,struct CSDdata *ptr) {
  ...
  nrng=ptr->nrang;
  nbyte=(nrng+7)/8;
  bnc=(ptr->bmnum & 0x0f) | (ptr->channel & 0x0f)<<4;
  ...
  size=sizeof(double)+2*nbyte+2+block+sizeof(int16)*11+sizeof(int32);
  if (ConvertWriteInt(fp,magic) !=0) return -1;
  if (ConvertWriteInt(fp,size) !=0) return -1;
  ...
  if (ConvertWriteShort(fp,ptr->intt.us) !=0) return -1;
  ...
  if (write(fp, (void *) &bnc,1) !=1) return -1;
...
char *CSDMakeBuffer(struct CSDdata *ptr,int *sze) {
  ...
  off+=2;
  ConvertFromShort(ptr->intt.us,buf+off);
  off+=2;
