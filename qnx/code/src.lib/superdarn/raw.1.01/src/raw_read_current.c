...
int raw_read_current(struct rawfp *fp,struct rawdata *raw_data) { 
   ...
   fp->rlen=0;
   do {
     if (read_short(fp->rawfp,&num_byte) !=0) {
       free(inbuf);
       return -1;
     }
     fp->rlen+=num_byte;
   ...
   /* copy radar_parms */   
   cnv_block(inbuf_ptr,radar_parms_pat);
   ...
   for (i=0;i<raw_data->PARMS.MPPUL;i++) {
      cnv_short(inbuf_ptr,&raw_data->PULSE_PATTERN[i]);
   ...
   /* copy the lag table */
    ...
    for(j=0;j < 2; ++j)
      for(i=0; i < raw_data->PARMS.MPLGS; ++i) {
         cnv_short(inbuf_ptr,&raw_data->LAG_TABLE[j][i]);
         inbuf_ptr = inbuf_ptr + sizeof(int16);
         num_byte = num_byte - sizeof(int16);
      } 
	  
   /* copy comment buffer */
   memcpy(raw_data->COMBF,inbuf_ptr,COMBF_SIZE);
   inbuf_ptr = inbuf_ptr + COMBF_SIZE;
   num_byte = num_byte - COMBF_SIZE;
  ...
  fp->ctime=time_epoch(raw_data->PARMS.YEAR,
