...
int OldFitSeek(struct OldFitFp *fp,
  ...
  if ((fp->stime !=-1) && (tval<fp->stime)) {
    if (fp->fitfp !=-1) lseek(fp->fitfp,fp->fit_recl,SEEK_SET);
    else fp->fptr=fp->fit_recl;
    fp->ctime=fp->stime;
  ...
  if ((fp->etime !=-1) && (tval>fp->etime)) {
    ...
    if (fp->fitfp !=-1) 
      ...
      else fp->fptr=(inx_rec[1]-1)*fp->fit_recl;
    fp->ctime=fp->etime;
    if (atme !=NULL) *atme=fp->ctime;
    return 3;
  }
 
  if ((fp->ibuf !=NULL) || (fp->inxfp !=-1)) {
  ...
  if (tval>=fp->ctime) {
    ...
    if (fp->fitfp !=-1) lseek(fp->fitfp,
                             -fp->fit_recl*(fp->blen+bjmp),SEEK_CUR);
    else fp->fptr-=fp->fit_recl*(fp->blen+bjmp);
  }
  ...
  if (fp->fitfp !=-1) lseek(fp->fitfp,
                             -fp->fit_recl*fp->blen,SEEK_CUR);

  else fp->fptr-=fp->fit_recl*fp->blen;
