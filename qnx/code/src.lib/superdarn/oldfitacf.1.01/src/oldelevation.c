...
double oldelevation(struct FitPrm *prm,double range,double phi0) {
 
  double k;          /* wave number; 1/m */
  double phi=0;      /* beam direction off boresight; rad */
  ...
  phi= prm->bmsep*(prm->bmnum - 7.5)* PI/ 180.0;
  c_phi= cos( phi);
