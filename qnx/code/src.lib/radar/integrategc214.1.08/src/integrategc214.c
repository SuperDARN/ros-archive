...
int IntegrateGC214(struct TSGprm *prm,int tsgid,int mplgs,int *lags,
  ...
  *flg=0;
  ...
  if (IntegrateGC214Pulse(buf,nsample+SAMPLE_DELAY,
  ...
  while (1) {
      ...
      if (status==0) smpcnt++;
      if (status==1) MonitorScan(monid,abuf,nsample+SAMPLE_DELAY,nchannel);
      ...
      if (status==1) {
        ...
        if (user !=NULL) (user)(prm,&tick,tock,nave,
		                bufadr[abuf],abuf,nsample+SAMPLE_DELAY,
                                nchannel,usrdata);

        ACFSumPower(prm,mplgs,lags,pwr0,
	            (int16 *) bufadr[buf],2,1,SAMPLE_DELAY,REAL_BUF_OFFSET,
		    IMAG_BUF_OFFSET,badrng,noise,MAXPWR,0.0,
                    thr,lmt,&abflg);
        ...
        if (abflg==0) {
          ...
          ACFCalculate(prm,(int16 *) bufadr[abuf],2,1,SAMPLE_DELAY,
                       REAL_BUF_OFFSET,IMAG_BUF_OFFSET,mplgs,
	  	       lags,acfd,ACF_PART,badrng,0.0,NULL);
          if (xcfd !=NULL) ACFCalculate(prm,(int16 *) bufadr[abuf],2,1,
				        SAMPLE_DELAY,REAL_BUF_OFFSET,
				        IMAG_BUF_OFFSET,mplgs,
		                        lags,xcfd,XCF_PART,
                                        badrng,0.0,NULL);
  ...
  status=GC214GetScanStatus(drid)==SCAN_OK;
  if (status==1) MonitorScan(monid,buf,nsample+SAMPLE_DELAY,nchannel);

  if (status==0) smpcnt++;
  if (status==1) { 
    if (user !=NULL) (user)(prm,&tick,tock,nave,
		            bufadr[buf],buf,nsample+SAMPLE_DELAY,
                            nchannel,usrdata);
    ACFSumPower(prm,mplgs,lags,pwr0,
	        (int16 *) bufadr[buf],2,1,SAMPLE_DELAY,REAL_BUF_OFFSET,
		IMAG_BUF_OFFSET,badrng,noise,MAXPWR,0.0,
                thr,lmt,&abflg);
    if (abflg==0) {
      ACFCalculate(prm,(int16 *) bufadr[buf],2,1,SAMPLE_DELAY,REAL_BUF_OFFSET,
		   IMAG_BUF_OFFSET,mplgs,
 	           lags,acfd,ACF_PART,badrng,0.0,NULL);
      if (xcfd !=NULL) ACFCalculate(prm,(int16 *) bufadr[buf],2,1,SAMPLE_DELAY,
				    REAL_BUF_OFFSET,IMAG_BUF_OFFSET,mplgs,
			            lags,xcfd,XCF_PART,badrng,0.0,NULL);
      nave++;
    }
  }
