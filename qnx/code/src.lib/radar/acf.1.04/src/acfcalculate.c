...
int ACFCalculate(struct TSGprm *prm,
		 ...
		 int mplgs,int *lagtable,
  	         float *acfbuf,
	         int xcf,int badrange,float atten,float *dco) {
   ...
   for(range=0;range < nrang ; range++) {
	 ...
	 else offset2 = ((range+sdelay) * rngoff) + 2;
	 ...
	 for(lag=0;lag < mplgs ; lag++) {
       ...
       if ((range >= badrange) && (lag == 0)) {
           ...
           sample1 =lagtable[mplgs-1]*sampleunit + offset1;        
           sample2 =lagtable[2*mplgs-1]*sampleunit + offset2;
       } else {  
           sample1 =lagtable[lag]*sampleunit + offset1;        
           sample2 =lagtable[mplgs+lag]*sampleunit + offset2;
       }
