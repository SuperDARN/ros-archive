...
#define END_DELAY (23000/CLOCK_PERIOD)	/* time from last pulse to end of tm seq */
...
void wrbuf(struct tsg_buf *tsg,
	       int code,int delay,struct tsg_cnt *cnt) {
  while (delay > 255) {	
	tsg->code_byte[tsg->len] = (char) code;
	tsg->rep_byte[tsg->len] = (char) 255;
	tsg->len++;
	delay = delay - 255;
  }
  tsg->code_byte[tsg->len] = (char) code;
  tsg->rep_byte[tsg->len] = (char) delay;
  tsg->len++;  
...
void free_tsg(struct tsg_buf *ptr) {
  if (ptr->code_byte !=NULL) free(ptr->code_byte);
  if (ptr->rep_byte !=NULL) free(ptr->rep_byte);
  free(ptr);
}

struct tsg_buf *make_tsg(struct tsg_parms *tsg,int *flg) {

  enum tsg_states state;
  int temp;
  int t2,t3,t4,t5,ttemp,ttemp1,smsp,lgfr,mpnc,txp,p_time,s_time,f_time;
  int phase_delay, phase_time_used,mlag;
  int i, j,buf_len;
  int  N_sample, ret_code;
 ...
 if ((buf=malloc(sizeof(struct tsg_buf)))==NULL) return NULL;  
  cnt.last=0;
  cnt.n_smp=0;
  ...
  if (tsg->mppul>0) {
      ...
      *flg=TSG_INV_MPINC_SMSEP;   
  	...
  	if ( ((20L*(long)(tsg->mppul)*(tsg->txpl))/
		 ...
		 (tsg->nrang)*(tsg->nbaud)*(tsg->smsep) + tsg->lagfr) ) >= 1L) {
      free(buf);
  ...
  */
  ...
  if (tsg->mppul == 0) {
    ...
    p_time = (tsg->pat)[i++]*mpnc + 1;  /* initialize time to next pulse event */
    s_time = p_time + lgfr + t2 + t3 +txp/2; /* init time to next sample event */
  ...
  if ((buf->code_byte = malloc(2*buf_len)) == NULL) {
    free(buf);
    *flg=TSG_INV_MEMORY;
    return NULL;
  } 
     
  if ((buf->rep_byte  = malloc(4*buf_len)) == NULL) {
    free(buf->code_byte);
    free(buf);
    *flg=TSG_INV_MEMORY;
    return NULL;
  ...
  ttemp = END_DELAY - (32767 - p_time);

  if ( (tsg->mppul == 0) || (ttemp <= 0)) ttemp = 1;
  wrbuf(buf,Null_code, ttemp,&cnt);
