...
int RplotInClipPointStart(char *name,int atnum,char **atname,char **atval,
		     char *buf,int sze,void *data) {

  struct RplotInBlock *blk;
  struct RplotInClipPoint *ptr;

  blk=(struct RplotInBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct RplotInClipPoint *) blk->data;
...
int RplotInClipPointEnd(char *name,char *buf,int sze,void *data) {
...
int RplotInClipStart(char *name,int atnum,char **atname,char **atval,
  ...
  struct RplotInBlock *blk;
  struct RplotInClip *ptr;
  struct RplotInPlot *plt;

  blk=(struct RplotInBlock *) data;
  if (blk==NULL) return -1;
  ptr=(struct RplotInClip *) blk->data;
  if (ptr==NULL) return -1;
  plt=ptr->plt;
  ...
  if (strcmp(name,"pp")==0) {
    ...
    pnt=malloc(sizeof(struct RplotInClipPoint));
    pnt->inc=&plt->inc;
    pnt->x=&(ptr->px[n]);
    pnt->y=&(ptr->py[n]);
    pnt->t=&(ptr->t[n]);
    ...
    XMLSetStart(blk->xml,RplotInClipPointStart,blk);
    XMLSetEnd(blk->xml,RplotInClipPointEnd,blk);
...
int RplotInClipEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (strcmp(name,blk->name)==0) {
    ...
    int i;
    if (ptr->num==0) {
       if (plt->clip.user !=NULL) s=(plt->clip.user)(0,NULL,
                                                     NULL,NULL,          
                                                     plt->clip.data);
       if (plt->clp.px !=NULL) free(plt->clp.px);
       if (plt->clp.py !=NULL) free(plt->clp.py);
       if (plt->clp.t !=NULL)  free(plt->clp.t);
       plt->clp.px=NULL;
       plt->clp.py=NULL;
       plt->clp.t=NULL;
       plt->clp.num=0;
    ...
    for (i=0;i<ptr->num;i++) {
      if (plt->matrix !=NULL) 
        RplotInMatrixTransform(plt->matrix,ptr->px[i],ptr->py[i],
                               &ptr->px[i],&ptr->py[i]);
      ptr->px[i]+=plt->x;
      ptr->py[i]+=plt->y;
    }
     
    if (plt->clip.user !=NULL) s=(plt->clip.user)(ptr->num,ptr->px,
                                                     ptr->py,ptr->t,
                                                     plt->clip.data);
