...
int RplotInInfoText(char *buf,int sze,void *data) {
  ...
  if (ptr->info.text.user !=NULL) return 
     (ptr->info.text.user)(buf,sze,ptr->info.text.data);
  return 0;
}
...
int RplotInInfoStart(char *name,int atnum,char **atname,char **atval,
  ...
  ptr=(struct RplotInPlot *) blk->data;
  if (ptr==NULL) return -1;
  if (ptr->info.start.user !=NULL) return
     (ptr->info.start.user)(name,atnum,atname,atval,
                            buf,sze,ptr->info.text.data);
...
int RplotInInfoEnd(char *name,char *buf,int sze,void *data) {
  ...
  if (ptr->info.end.user !=NULL) return
     (ptr->info.end.user)(name,buf,sze,ptr->info.text.data);
