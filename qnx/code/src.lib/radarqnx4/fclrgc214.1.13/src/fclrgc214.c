...
/*
 $Log: fclrgc214.c,v $
 Revision 1.13  2007/04/19 18:03:20  code
 Changed bandwidth to 0.8MHz.

 Revision 1.12  2005/07/14 19:19:04  barnes
 Moved the offsets of the real and imaginary parts to arguments.

 Revision 1.11  2005/07/14 19:12:29  barnes
 Modification to reverse the sample ordering for HF mode radars.

 Revision 1.10  2005/07/06 18:09:55  barnes
 Added Dieter's fix to correct the FFT for radars operating in the HF.

 Revision 1.9  2005/06/16 18:43:21  barnes
 Added Dieter Andre's code for operation at HF.

 Revision 1.8  2004/09/01 14:23:23  barnes
 Opened bandwidth of search to 1.5 MHz to take advantage of the physical
 filter.
...
int FCLRGC214(ipcid_t dioid,ipcid_t drid,struct FreqTable *fptr,
  ...
  for (l=0;l<LOOP;l++) {
     ...
     if (status !=SCAN_OK) return -1;
      ...
      for (n=0;n<nn;n++) fft[1+2*n]=(float) ptr[2*n+roff+2*c*nn];
      for (n=0;n<nn;n++) fft[1+2*n+1]=(float) ptr[2*n+ioff+2*c*nn]; 
 
      fft1(fft,nn,1);
      
      pwr[0]+=fft[1]*fft[1]+fft[2]*fft[2];
      if (Fclk==40625000) {
        for (n=1;n<nn;n++) pwr[   n]+=fft[1+2*n]*fft[1+2*n]+
