...
/*
 ...
 Revision 1.10  2008/03/21 14:27:21  code
 Fixed bug in setting pointers.

 Revision 1.9  2008/03/20 21:43:48  code
 Added IQCopy.

 Revision 1.8  2008/03/18 23:28:23  code
 Fixed bug in setting the skip sample number.

 Revision 1.7  2008/03/18 21:47:20  code
 Removed redundant lagmax variable.

 Revision 1.6  2008/03/14 20:07:04  code
 Fixed the setting of the IQ sample data.

 Revision 1.5  2008/03/14 18:59:42  code
 Removed the rawdump features and added code to pass the IQ samples back up to the control program.
...
int IntegrateGC214ex(struct TSGprm *prm,int tsgid,int mplgs,
		   ...
		   void (*user)(struct TSGprm *prm,
			        ...
			        int nave,int atten,float *noise,
 ...
 */
  ...
  *skpnum=prm->smdelay;
  ...
  if ((s=IntegrateGC214exPulse(buf,nsample,
  ...
  while (1) {
	...
	mark[snum].tval.tv_sec=tick.tv_sec;
        ...
        mark[snum].off=iqsze;
        mark[snum].sze=(nsample*nchannel)*4;
        mark[snum].atten=0;
        snum++;
      }
      ...
      if (status==1) {
	 if (iqbuf==NULL) iqbuf=malloc(4*(nsample*nchannel));
         else {
	   iqtmp=realloc(iqbuf,iqsze+4*(nsample*nchannel));
           if (iqtmp==NULL) {
             free(iqbuf);
             return -1;
	   }
         ...
         memcpy(iqbuf+iqsze,bufadr[abuf],4*(nsample*nchannel));
  ...
  if (status==1) { 
    if (iqbuf==NULL) iqbuf=malloc(4*nsample*nchannel);
      ...
      memcpy(iqbuf+iqsze,bufadr[buf],4*(nsample*nchannel));
      if ((samples !=NULL) && ((iqsze+(nsample*nchannel)*4)<IQBUFSIZE)) {
  ...
  if (nave>0) {
    ACFexCalculate(prm,(int16 *) iqbuf,iqnum,nave,nsample,
                   roff,ioff,
