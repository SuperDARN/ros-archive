...
/*
 $Log: integrates.c,v $
 Revision 1.14  2008/03/20 20:49:00  code
 Added IQCopy.

 Revision 1.13  2008/03/18 21:43:52  code
 Removed redundant lagmax variables.

 Revision 1.12  2008/03/15 14:56:42  code
 In stereo the I&Q buffer is split between stereo channels so it only
 have as big.

 Revision 1.11  2008/03/14 19:02:30  code
 Fixed bug in setting the IQ buffer size.

 Revision 1.10  2008/03/14 16:47:31  code
 Changed variable name.

 Revision 1.9  2008/03/14 16:04:21  code
 Added sample skip number.

 Revision 1.8  2008/03/14 15:47:10  code
 Added check for NULL pointer to I&Q sample buffer.

 Revision 1.7  2008/03/13 23:35:53  code
  ...
  *rxchnB=nchannelB;
      ...
      if (statusA==1) {
        ...
        if ((samplesA !=NULL) && 
           ...
           IQCopy( (int16 *) bufadr[abuf],samplesA+iqoffA,
                 nsampleA,nchannelA,REAL_BUF_OFFSET,IMAG_BUF_OFFSET,
                 2*nchannelA,2);
        ...
        if (userA !=NULL) (userA)(prmA,&tick,tock,*naveA,(*attenA)*atstpA,
		                noiseA,bufadr[abuf],abuf,nsampleA,nchannelA,
						      usrdataA);
          
        aflgA=ACFSumPower(prmA,mplgsA,lagsA,pwr0A,
		         (int16 *) bufadr[abuf],2*nchannelA,0,
        ...
        if (abflgA==0) {
          ...
          ACFCalculate(prmA,(int16 *) bufadr[abuf],2*nchannelA,0,
      ...
      if (statusB==1) {
        ...
        if ( (samplesB !=NULL) && 
           ...
           IQCopy( (int16 *) bufadr[bufoff+abuf],samplesB+iqoffB,
                 nsampleB,nchannelB,REAL_BUF_OFFSET,IMAG_BUF_OFFSET,
                 2*nchannelB,2);
        ...
        if (userB !=NULL) (userB)(prmB,&tick,tock,*naveB,(*attenB)*atstpB,
		                noiseB,bufadr[bufoff+abuf],
                                abuf,nsampleB,nchannelB,
					       usrdataB);
          
          
        aflgB=ACFSumPower(prmB,mplgsB,lagsB,pwr0B,
		         (int16 *) bufadr[bufoff+abuf],2*nchannelB,0,
        ...
        if (abflgB==0) {
          ...
          ACFCalculate(prmB,(int16 *) bufadr[bufoff+abuf],2*nchannelB,0,
  ...
  if (statusA==1) { 
    ...
    if ( (samplesA !=NULL) && 
     ...
     if (userA !=NULL) (userA)(prmA,&tick,tock,*naveA,(*attenA)*atstpA,
		             noiseA,bufadr[buf],buf,nsampleA,nchannelA,
			     usrdataA);


     aflgA=ACFSumPower(prmA,mplgsA,lagsA,pwr0A,
		         (int16 *) bufadr[buf],2*nchannelA,0,
                         REAL_BUF_OFFSET,IMAG_BUF_OFFSET,badrngA,
                         *noiseA,mxpwrA,catnA*atstpA,thr,lmt,&abflgA);
     ...
     if (abflgA==0) {
        ...
        ACFCalculate(prmA,(int16 *) bufadr[buf],2*nchannelA,0,
                     REAL_BUF_OFFSET,IMAG_BUF_OFFSET,mplgsA,lagnumA,
	  	     lagsA,acfdA,ACF_PART,2,badrngA,catnA*atstpA,dptr);
  ...
  if (statusB==1) { 
    ...
    if ((samplesB !=NULL) && 
     ...
     if (userB !=NULL) (userB)(prmB,&tick,tock,*naveB,(*attenB)*atstpB,
		             noiseB,bufadr[bufoff+buf],buf,nsampleB,nchannelB,
			     usrdataB);


     aflgB=ACFSumPower(prmB,mplgsB,lagsB,pwr0B,
		         (int16 *) bufadr[bufoff+buf],2*nchannelB,0,
                         REAL_BUF_OFFSET,IMAG_BUF_OFFSET,badrngB,
                         *noiseB,mxpwrB,catnB*atstpB,thr,lmt,&abflgB);
     ...
     if (abflgB==0) {
        ...
        ACFCalculate(prmB,(int16 *) bufadr[bufoff+buf],2*nchannelB,0,
                     REAL_BUF_OFFSET,IMAG_BUF_OFFSET,mplgsB,lagnumB,
	  	     lagsB,acfdB,ACF_PART,2,badrngB,catnB*atstpB,dptr);
