...
/*
 $Log: main.c,v $
 Revision 1.6  2008/03/12 17:44:41  code
 Fixes from Blackstone testing.

 Revision 1.5  2008/03/12 14:17:47  code
 Blackstone code fixes from Todd.

 Revision 1.2  2007/05/17 19:03:21  code
 Initial revision
...
/* 
   #define GAIN_ADJ 3
   #define FINAL_SHIFT_ADJ 5
*/

#define GAIN_ADJ 1
#define FINAL_SHIFT_ADJ 0
...
*/
	...
	for(x=0;x<smpdlyCD;x++) tmp=read32(baseGC214+0x60000);
           DMAxfer(BASE2,dmabuf[5].phys_addr,0x60000,4*metsample);
	   if (DMApoll(BASE2)==-1){
		DMAabort(BASE2);
#ifdef LOGERROR
		tod=time(NULL);
