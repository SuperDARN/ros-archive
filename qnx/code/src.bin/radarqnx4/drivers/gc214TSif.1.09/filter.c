...
/*
 $Log: filter.c,v $
 Revision 1.3  2008/03/12 15:06:38  code
...
void blackman(float *blackman,int N){
  ...
  M=(N-1)/2;
  for(n=0;n<N;n++){
    blackman[n]=0.42 + 0.5*cos( (2*PI*((float)n-M))/(2*M+1) ) + 
                0.08*cos( (4*PI*((float)n-M))/(2*M-1) );
  }
}
...
float setCFIR(int *cfircoeffs,float freq_in,float Fpass,float Fstop) {
 ...
 /* if CFIR gain > 1, and large signals, then overflows occur.  
    To absoluteley prevent overflows, coeffs must be 
    chosen such that CFIRgain<=1 (see pg 8 in gc4016 manual) */
...
float setPFIR(int *pfircoeffs,float freq_in,float Fpass,float Fstop,
              int matched) {
  ...
  */
  ...
  if(matched){
    ...
    for(n=0;n<31;n++){
      pfircoeffs[n]=(int)(32767*window[n]*(pi/wc)*
                    sin(wc*((float)n-31))/(pi*((float)n-31))+.49999);
      PFIRgain=PFIRgain+pfircoeffs[n];
    }
    pfircoeffs[31]=32767;
    PFIRgain=2*PFIRgain+pfircoeffs[31];
 ...
 /* if PFIR gain > 1, and large signals, then overflows occur.  
    To absoluteley prevent overflows, coeffs must be chosen such that 
    PFIRgain<=1 
  */
...
float setresamp(int *resampcoeffs,float freq_in,float Fpass,float Fstop,
                int filtnum,int bypass){
  ...
  */
  ...
  if(bypass){
    ...
    for(n=0;n<128;n++){
      resampcoeffs[2*n+filtnum]=(int)(2047*window[n]*(pi/wc)*sin(wc*
                              ((float)n-127.5))/(pi*((float)n-127.5))+.49999);
    ...
    if(resampgain>1){
      ...
      resampgain=0;
      for(n=0;n<128;n++){
        resampcoeffs[2*n+filtnum]=(int)( (1/gaintemp)*(
                              float) resampcoeffs[2*n+filtnum]+0.499999 );
        resampgain=resampgain+resampcoeffs[2*n+filtnum];
      }
      resampgain=2*resampgain/32768;
    }
