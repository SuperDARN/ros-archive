...
int reset(int stereo) {
  ...
  PUT_DIO(hdw.Atten_port[0], 0x70, hdw.Atten_mask[0]);
  PUT_DIO(hdw.TSG_port[0], 0x00, 0xFF); 
  if (stereo !=0) {
    PUT_DIO(hdw.Atten_port[1], 0x70, hdw.Atten_mask[1]); 
    PUT_DIO(hdw.TSG_port[1], 0x00, 0xFF); 
  }
  return 0;
}
