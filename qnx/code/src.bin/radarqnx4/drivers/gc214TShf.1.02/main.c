...
/*
 $Log: main.c,v $
 Revision 1.2  2007/10/29 20:50:54  code
 Modification to support multiple cards.
...
int mosf=METEOR_OVERSAMP_FACTOR;
...
void setcard(int Fsamp,int Fstop,int Fpass,float freqout) {
  ...
  */
  ...
  /*
    CFIRgain=setCFIR(cfircoeffsCD,CFIRfreq,2*freqoutCD,2*freqoutCD);
    PFIRgain=setPFIR(pfircoeffsCD,PFIRfreq,freqoutCD,freqoutCD,1);
    resampgain=setresamp(resampcoeffs,resampfreq,freqoutCD,freqoutCD,1,
    RESAMP_BYPASS);
  */
...
int burstscan() { 
  ...
  while (1) {    
    /* Check to see if at least one frame has been collected, 
       if so, make sure there was no FIFO overrun, and exit */
...
#ifdef LOGERROR
        tod=time(NULL);
	fprintf(gcerrf,"FIFO Overflow: %s\n",ctime(&tod));
#endif
...
#ifdef LOGERROR
        tod=time(NULL);
	if(fclrflag) fprintf(gcerrf,"FCLR CH1 A/D Over-range: %s\n",
                             ctime(&tod));
#endif
...
#ifdef LOGERROR
        tod=time(NULL);
	if(fclrflag) fprintf(gcerrf,"FCLR CH2 A/D Over-range: %s\n",
                             ctime(&tod));
#endif
	/* return SCAN_OVERFLOW; */
	return SCAN_OK;
    ...
    /* Check to see if this is taking too long (this can be due to 
       too much data being requested, or to a lack of trigger */
...
#ifdef LOGERROR
      tod=time(NULL);
      fprintf(gcerrf,"Did not collect data in alloted time: %s\n",ctime(&tod));
#endif
      return SCAN_ERROR;
    }
    delay(1);
...
void main(int argc,char *argv[]) {
  ...
  */
  ...
  OptionAdd(&opt,"pci",'i',&pci_index);

  arg=OptionProcess(1,argc,argv,&opt,NULL);   
  if (arg<argc) strcpy(driver_name,argv[arg]);
#ifdef LOGERROR
  gcerrf=fopen("/data/errlogs/gc214TShf.err","a");
#endif

  fprintf(stderr,"\nGC214TShf digital receiver driver, version: %d.%.2d\n",
         MAJOR_VERSION,MINOR_VERSION);

  if (debug !=0) fprintf(stderr,"DEBUG MODE\n"); 
  fprintf(stderr," Registered under the name: %s\n",driver_name); 
  fprintf(stderr," Number of DMA Buffers:%d\n",dmabufnum);

  timeofday=time(NULL);
  fprintf(stderr," driver started at %s\n",ctime(&timeofday));

#ifdef LOGERROR
  fprintf(gcerrf,"GC214TShf digital receiver driver, version: %d.%.2d\n",
          MAJOR_VERSION,MINOR_VERSION);

  if (debug !=0) fprintf(gcerrf,"DEBUG MODE\n"); 
  
 
  fprintf(gcerrf," Registered under the name: %s\n",driver_name); 
  fprintf(gcerrf," Number of DMA Buffers:%d\n",dmabufnum);
  fprintf(gcerrf," driver started at %s\n",ctime(&timeofday));
  fprintf(stderr," Error output directed to /data/errlogs/gc214TShf.err\n"); 
  fflush(gcerrf);
#endif
  ...
  if (debug==0) {
    ...
    if((write_PCI9080_CNTRL=initPCI9080(pci_index,&irq,
    ...
    baseGC214pointer=mmap(0,1048576,PROT_EXEC|PROT_READ|PROT_WRITE,
                          MAP_SHARED,fd,BASE3);
...
#ifdef LOGERROR
    fprintf(gcerrf,"Unable to set system clock resolution to 1 ms");
#endif
    exit(1);
  }
...
#ifdef LOGERROR
  fprintf(gcerrf," system clock resolution set to %d nsec\n",res.tv_nsec);
#endif
...
/*
    BEGIN TEST CODE TO LOOK AT FILTER REPONSES
    fp=fopen("test.out","w");
    matched=1;
    for(tfreq=9900000;tfreq<=10100000;tfreq+=100){
	 printf("freq=%d\n",tfreq);
	 sample=50;
	 freqout=3333.33333;
         chana.freq= pow( 2, 32)* 
         ...
         setcard(Fsamp,Fstop,Fpass,freqout);
         setupGC214(baseGC214,0,0,sample+smpdlyAB+20,metsample+smpdlyCD+20,0);
	 ...
	 for(x=0;x<(smpdlyAB+20);x++) tmp=read32(baseGC214+0x00000);
	 for(x=0;x<(smpdlyCD+20);x++) tmp=read32(baseGC214+0x40000);
	 power=0;
	 for(x=0;x<sample;x++){
		tmp=read32(baseGC214+0x40000);
		I=tmp&0x0000ffff;
		Q=tmp>>16;
		power+=(float)(I*I+Q*Q);
	 }
...
*/
  ...
  while (1) {
    ...
    case SET_SRATE:
       ...
       Replymx(pid,1,msg);
       break;
    case SET_METEOR_SRATE:
       _setmx(&msg[0],&freqoutCD,sizeof(freqoutCD));  
       Readmsgmx(pid,sizeof(code),1,msg);
       ...
       Replymx(pid,1,msg);
       break;
    case SET_METEOR_SAMPLE:
       _setmx(&msg[0],&metsample,sizeof(metsample));  
       Readmsgmx(pid,sizeof(code),1,msg);
    ...
    case SET_TX_FREQ:
...
#ifdef LOGERROR
	 tod=time(NULL);
	 fprintf(gcerrf,"More than 4096 samples requested: %s\n",ctime(&tod));
#endif 
	 break;
       }		
       if(sample<0){
...
#ifdef LOGERROR
	 tod=time(NULL);
	 fprintf(gcerrf,"Less than 0 samples requested: %s\n",ctime(&tod));
#endif 
	 break;
       }		
       if(metsample>8192){
	 ...
	 tod=time(NULL);
	 fprintf(gcerrf,
          "More than 8192 meteor echo samples requested, data truncated: %s\n",
          ctime(&tod));
#endif
       }
       if(metsample<0){
	 ...
	 tod=time(NULL);
	 fprintf(gcerrf,
           "Less than 0 meteor echo samples requested, data truncated: %s\n",
           ctime(&tod));
#endif
       }
	 ...
	 if (MATCHED) matched=1;
         ...
         setcard(Fsamp,Fstop,Fpass,freqout);
         setupGC214(baseGC214,0,0,sample+smpdlyAB,metsample+smpdlyCD,0);
         setupGC4016(baseGC214,&global, resampcoeffs);
         setupGC4016channel(baseGC214,0,&chana,cfircoeffsAB,pfircoeffsAB);
         setupGC4016channel(baseGC214,1,&chanb,cfircoeffsAB,pfircoeffsAB);
         setupGC4016channel(baseGC214,2,&chanc,cfircoeffsCD,pfircoeffsCD);
       ...
       if (debug==0) {
         ...
         if ( (scanstatus==SCAN_OK) ) {
           x=0;
           clock_gettime(CLOCK_REALTIME,&start);
	   for(x=0;x<smpdlyAB;x++) tmp=read32(baseGC214+0x00000);
           DMAxfer(BASE2,dmabuf[buffer].phys_addr,0x00000,4*sample);
	   if (DMApoll(BASE2)==-1){
		DMAabort(BASE2);
#ifdef LOGERROR
		tod=time(NULL);
		fprintf(gcerrf, 
                  "DMA transfer did not complete: %s\n",
                  ctime(&tod));
#endif
		scanstatus=SCAN_ERROR;
	   } 
	   for(x=0;x<smpdlyAB;x++) tmp=read32(baseGC214+0x20000);
           DMAxfer(BASE2,dmabuf[buffer].phys_addr+4*sample,0x20000,4*sample);
	   if (DMApoll(BASE2)==-1){
		DMAabort(BASE2);
#ifdef LOGERROR
		tod=time(NULL);
		fprintf(gcerrf, "DMA transfer did not complete: %s\n",
                        ctime(&tod));
#endif
		scanstatus=SCAN_ERROR;
	   } 
	   for(x=0;x<smpdlyCD;x++) tmp=read32(baseGC214+0x40000);
           DMAxfer(BASE2,dmabuf[4].phys_addr,0x40000,4*metsample);
	   if (DMApoll(BASE2)==-1){
		DMAabort(BASE2);
#ifdef LOGERROR
		tod=time(NULL);
	   ...
	   for(x=0;x<smpdlyCD;x++) tmp=read32(baseGC214+0x60000);
           DMAxfer(BASE2,dmabuf[5].phys_addr+4*sample,0x60000,4*metsample);
	   if (DMApoll(BASE2)==-1){
		DMAabort(BASE2);
#ifdef LOGERROR
		tod=time(NULL);
...
#ifdef LOGERROR
		tod=time(NULL);
		fprintf(gcerrf,"SCAN ERROR status=%d: %s\n",status,
                        ctime(&tod));
#endif
	 }
...
#ifdef LOGERROR
	   tod=time(NULL);
	   fprintf(gcerrf,"More than 8192 samples requested in FCLR: %s\n",
                          ctime(&tod));
#endif
	 }
	 if(sample<0){
...
#ifdef LOGERROR
	   tod=time(NULL);
	   fprintf(gcerrf,"Less than 0 samples requested in FCLR: %s\n",
                   ctime(&tod));
#endif
	 }
	   ...
	   matched=0;
  	   /* filter corner frequency=mean of setcard args 2&3 */
           ...
           setupGC214(baseGC214,0,0,sample+smpdlyAB+30,metsample+smpdlyCD,0);
           setupGC4016(baseGC214,&global,resampcoeffs);
           setupGC4016channel(baseGC214,0,&chana,cfircoeffsAB,pfircoeffsAB);
           setupGC4016channel(baseGC214,1,&chanb,cfircoeffsAB,pfircoeffsAB);
           setupGC4016channel(baseGC214,2,&chana,cfircoeffsAB,pfircoeffsAB);
           setupGC4016channel(baseGC214,3,&chanb,cfircoeffsAB,pfircoeffsAB);
           releaseGC4016(baseGC214);
           enableGC214(baseGC214);
	   ...
	   fclrflag=1;
 	   ...
 	   fclrflag=0;
           if (scanstatus!=SCAN_ERROR) {
             clock_gettime(CLOCK_REALTIME,&start);
	     for(x=0;x<smpdlyAB+30;x++) tmp=read32(baseGC214+0x00000);
             DMAxfer(BASE2,dmabuf[buffer].phys_addr,0x00000,4*sample);
	     if (DMApoll(BASE2)==-1){
	  	DMAabort(BASE2);
#ifdef LOGERROR
