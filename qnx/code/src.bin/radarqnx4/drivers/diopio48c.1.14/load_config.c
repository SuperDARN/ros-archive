...
/*
 $Log: load_config.c,v $
 Revision 1.6  2007/11/13 19:00:42  code
 Fixed variable name.
...
int load_config(FILE *fp) {
   ...
   while (fgets(line,1024,fp) !=0) {
    ...
    if (strcmp(cmd,"name")==0) {
    ...
    } else if (strcmp(cmd,"phase_port")==0) {
      op=strtok(NULL,"\n");
      if (op !=NULL) {
        if (op[0] !='x') sscanf(op,"%x",&hdw.PHASE_port[channel]);
        else hdw.PHASE_port[channel]=-1;
      }
    } else if (strcmp(cmd,"phase_mask")==0) {
      op=strtok(NULL,"\n");
      if (op !=NULL) {
        if (op[0] !='x') sscanf(op,"%x",&hdw.PHASE_mask[channel]);
        else hdw.PHASE_mask[channel]=0;
      }
    ...
    } else if (strcmp(cmd,"widefilter_port")==0) {
      op=strtok(NULL,"\n");
      if (op !=NULL) {
        if (op[0] !='x') sscanf(op,"%x",&hdw.WIDEFILTER_port[channel]);
        else hdw.WIDEFILTER_port[channel]=-1;
      }
    } else if (strcmp(cmd,"widefilter_mask")==0) {
      op=strtok(NULL,"\n");
      if (op !=NULL) {
        if (op[0] !='x') sscanf(op,"%x",&hdw.WIDEFILTER_mask[channel]);
        else hdw.WIDEFILTER_mask[channel]=0;
      }
      ...
      else if (strcmp(cmd,"tx_inv")==0) hdw.TX_inv=1;
