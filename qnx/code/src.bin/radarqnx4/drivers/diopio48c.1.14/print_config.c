...
/*
 ...
 Revision 1.4  2007/11/09 21:59:36  code
 Added wide filter element.
...
void print_config(FILE *fp,int stereo) {
  ...
  for (i=0;i<(1+stereo);i++) {
    ...
    if (hdw.WIDEFILTER_port[i] !=-1) 
      fprintf(fp,"widefilter_port[%d]=%.2x,mask=%.2x\n",i,
                                    hdw.WIDEFILTER_port[i],
				    hdw.WIDEFILTER_mask[i]);
    else fprintf(fp,"widefilter_port[%d]=undefined\n",i); 
