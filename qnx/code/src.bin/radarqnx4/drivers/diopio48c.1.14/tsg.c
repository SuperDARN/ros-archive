...
/*
 $Log: tsg.c,v $
 Revision 1.6  2007/05/18 18:23:43  code
 Moved Scope Sync to the mktsg routine.

 Revision 1.5  2006/11/02 22:35:46  code
  ...
  if (status ==0) {
    ...
    /* download the sequence */
	 ...
	 for (i=0;i<header.length;i++) {
           if (hdw.TX_inv !=0) { /* invert the TX pulse */
	     unsigned char tmp,flg;
             flg=code[header.seq][i] & 0x04;
             tmp=code[header.seq][i] & 0xfb;
             if (flg !=0) code[header.seq][i]=tmp;
             else code[header.seq][i]=tmp | 0x04;
	   }
	   code[header.seq][i]=code[header.seq][i];
...
int dumptsg(FILE *fp,unsigned char seq) {
  int c;
  fprintf(fp,"%d %d %d\n",seq,tsg_length[seq],tlen[seq]);
  for (c=0;c<tsg_length[seq];c++) {
    fprintf(fp,"%d %d %d\n",c,code[seq][c],repeat[seq][c]);
  }  
  return 0;
}
...
int send_seq_mono(int channel,
  ...
  if (param.phased==0) put_beam(channel,param.beam[channel]);
  else put_phase(channel,param.phase[channel]);
...
int send_seq_stereo(unsigned char *seq_id,unsigned int *tsg_delay,
  ...
  if (param.phased==0) put_beam(0,param.beam[0]);
  else put_phase(0,param.phase[0]);
  if (param.phased==1) put_beam(1,param.beam[1]);
  else put_phase(1,param.phase[1]);
