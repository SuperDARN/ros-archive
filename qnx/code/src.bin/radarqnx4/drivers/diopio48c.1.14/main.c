/* main.c
   ======
   Author: R.J.Barnes, Julian Thornhill & A. Sessai Yukimatu
*/
...
/*
 $Log: main.c,v $
 Revision 1.9  2008/03/12 15:36:24  code
 Added command line options "-sn node" to set the node of the synthesizer.
 Also added "-switch freq" to set the frequency at which the test mode line
 is triggered.

 Revision 1.8  2007/11/09 21:56:34  code
 Added digital synth code.

 Revision 1.7  2006/11/02 22:35:46  code
 Added code to dump out the bit pattern of the pulse sequence.

 Revision 1.6  2005/06/16 18:37:02  barnes
...
extern int synthnode;

int synthswitch=12000;
...
void read_phase(pid_t pid) {
  ...
  reply_msg.phase=param.phase[param.channel];
  reply_msg.op_type=DIO_OK;
...
void set_phase(pid_t pid) {
  ...
  status=(Readmsg(pid,sizeof(char),&phase,sizeof(phase))!=sizeof(phase));
  if (param.phased==0) {
    phase=DIO_ERR;
    Reply(pid,&phase,sizeof(phase));
    return;
  }
  status=(Readmsg(pid,2*sizeof(char),&beam,sizeof(beam))!=sizeof(beam)); 
  if (status==0) status=put_phase(param.channel,phase);
  if (status !=0) phase=DIO_ERR;
  else {
    param.phase[param.channel]=phase;
    param.beam[param.channel]=beam;
    phase=DIO_OK;
    show_values();
  }
  Reply(pid,&phase,sizeof(phase));
...
void set_Hz(pid_t pid) {
  ...
  if (param.synth) {
    if (param.stereo==0) txsynth(freq);
    else txsynth_s(param.channel,freq);
    if(freq<synthswitch) {
      put_test_mode(param.channel,1);
      param.test_mode[param.channel]=1;
    } else {
      put_test_mode(param.channel,0);
      param.test_mode[param.channel]=0;
    }
  }   
...
void main(int argc,char *argv[]) {
  ...
  unsigned int pci=0; /* flag for PCI card */
  ...
  OptionAdd(&opt,"pci",'x',&pci);

  OptionAdd(&opt,"q",'x',&silent);
  OptionAdd(&opt,"s",'x',&param.stereo);  
  OptionAdd(&opt,"ph",'x',&param.phased);
  OptionAdd(&opt,"synth",'x',&param.synth);
  OptionAdd(&opt,"switch",'i',&synthswitch);  
  OptionAdd(&opt,"sn",'i',&synthnode);
  ...
  OptionAdd(&opt,"l",'x',&logseq);
  OptionAdd(&opt,"ld",'t',&logname);
  ...
  if (param.phased==1) {
    ...
    /* signal logic is inverted for phase steered radars */
  ...
  if (pci !=0) { /* PCI card being used - must be detected */
    int status;
    status=locate_card(&dio_adr[0],&dio_adr[1]);
    if (status<1) {
      if (status==0) fprintf(stderr,"No DIO cards located.\n");
      if (status==-1) fprintf(stderr,"No DIO cards located.\n");
      if (status==-2) fprintf(stderr,"No PCI BIOS found.\n");
      if (status==-3) fprintf(stderr,"Wrong vendor ID.\n");
      if (status==-4) fprintf(stderr,"Invalid card configuration.\n");
      exit(-1);
    }
  }
  ...
  init_tsg();
  zero_status();
  if (param.phased==0) put_beam(0,8);
  ...
  if (param.stereo==1) {
    if (param.phased==0) put_beam(1,8);
  ...
  while (wait) {
	...
	set_test_mode(pid);
	  ...
	  int yr,mo,dy,hr,mt,sc,us;
          ...
          sprintf(logfname,"%s/%.4d%.2d%.2d.%.2d%.2d.%.2d.%.2d.tsg",logdir,yr,
                  mo,dy,hr,mt,sc,tsg);
          fp=fopen(logfname,"w");
          dumptsg(fp,tsg);
          fclose(fp);
