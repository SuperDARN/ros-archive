...
int opensock(int sock,fd_set *fdset) {
  ...
  if (i>=CLIENT_MAX) { 
    ...
    ErrLog(tsk,taskname,"Too many clients attached - refusing connection.");
  ...
  if ((client[i].sock=accept(sock,
     ...
     ErrLog(tsk,taskname,"Accept failed.");
  ...
  if ((status=fcntl(client[i].sock,F_GETFL))==-1) {
    ...
    ErrLog(tsk,taskname,"Failed to read file control block.");
  ...
  if ((status=fcntl(client[i].sock,F_SETFL,status))==-1) {
    ...
    ErrLog(tsk,taskname,"Failed to write file control block.");
