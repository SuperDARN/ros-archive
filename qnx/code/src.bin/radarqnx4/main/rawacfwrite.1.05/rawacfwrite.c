...
/*
 $Log: rawacfwrite.c,v $
 Revision 1.5  2004/07/06 23:41:26  barnes
 Fixed bug discovered by Dieter Andre - RawFwrite returns -1 on error.

 Revision 1.4  2004/06/04 14:54:38  barnes
 Adopted text strings as radar identifiers.

 Revision 1.3  2004/05/13 22:24:15  barnes
 Added a reply to the quit message.

 Revision 1.2  2004/05/11 14:51:37  barnes
 Changed version handling.

 Revision 1.1  2004/05/10 15:37:47  barnes
 Initial revision
    ...
    if (msg==TASK_DATA) {
      ...
      if (filename !=NULL) {
        ...
        if (fp==NULL) ErrLog(tsk,taskname,"Error opening file.");
          ...
          for (dptr=0;dptr<dnum;dptr++) {
          dmsg[dptr].rdt->thr=thr;
          status=RawFwrite(fp,dmsg[dptr].prm,
                              dmsg[dptr].rdt);  
          if (status==-1) break;
	  }
