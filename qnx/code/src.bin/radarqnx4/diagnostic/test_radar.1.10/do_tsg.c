...
/* 
 $Log: do_tsg.c,v $
 Revision 1.5  2004/05/10 14:06:10  barnes
...
int test_tsg(char *arg) {
  ...
  for (i=0;i<repnum;i++) {
    if ((frqflg[0]==1) || (frqflg[1]==1)) 
       status=DIOSendTSG(dioid,stereo,seq,xflg,sflg,dptr,&fptr);
    else status=DIOSendTSG(dioid,stereo,seq,xflg,sflg,dptr,NULL);
    if (status !=0) break;
  }  
  return status;
}
