...
int test_filter(char *arg) {
  ...
  return set_filter(dio_id,filter);
}
...
int test_channel(char *arg) {
  ...
  return set_channel(dio_id,channel);
}
...
int test_antenna(char *arg) {
  ...
  return set_antenna(dio_id,mode,anten);
}
...
int test_mode(char *arg) {
  ...
  return set_test_mode(dio_id,mode);
}

void test_fstatus(char *arg) {
  int mode;
  mode=read_fmode(dio_id);
...
void test_status(char *arg) {
  ...
  status=get_dio_status(dio_id,clr,full,lstat,astat);
  if (status<0) fprintf(stdout,"Command:Failed\n");
  else {
    fprintf(stdout,"Command:Ok\n");
...
int test_fclr(char *arg) {
 ...
 for (j=0;j<stereo+1;j++) { 
  ...
  return send_fclr(dio_id,stereo,seqid,delay,fnum,fptr);
...
int load_tsg(char *arg) {
  ...
  if (ex !=0) return set_tsg(dio_id,tsgid,cnt,code,rep,xtd); 
  status=set_tsg(dio_id,tsgid,cnt,code,rep,NULL);
  return status;
}
...
int test_proxy(char *arg) {
  ...
  return scan_reset(ad_id,reset_id);
}

void test_ad_stat(char *arg) {
  int status=0;
  status=get_scan_status(ad_id);
...
int test_transfer(char *arg) {
  ...
  return do_scan(ad_id,bval,size,mode,channel);
}
