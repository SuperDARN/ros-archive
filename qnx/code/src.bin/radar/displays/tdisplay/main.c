        ...
        for (i=0;i<ptr->block.num;i++) {
          switch (ptr->block.data[i].type) {
            case FIT_TYPE: /* display fitted data */
		      ...
		      if (extra !=0) 
                      plot_param(rid,&fdt->prms,text,data,prg_ptr);
              else plot_param(rid,&fdt->prms,NULL,NULL,prg_ptr);
	      ...
	      case RAW_TYPE : /* display the ACF data */	
            ...
            if (mode == -1) {
		      ...
		      if (extra !=0) 
                        plot_param(rid,&rdt->PARMS,text,data,prg_ptr);
              else plot_param(rid,&rdt->PARMS,NULL,NULL,prg_ptr);
              plot_acf(rdt,range);
		      changed=0;
