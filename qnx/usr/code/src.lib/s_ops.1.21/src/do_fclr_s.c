...
/*
 $Log: do_fclr_s.c,v $
 Revision 1.11  2002/03/26 20:15:50  barnes
 Tidied up code - possible screw up in version numbers.
...
/*
	for (i=0;i<NUM_FCLR_FREQ;i++)
	{
		...
		{
			if (i==0)
				lowest_freqB[i]=lowest_freqB[i+1];
			else
				lowest_freqB[i]=lowest_freqB[i-1];
		}
	}
...
// remove any channel B frequencies within AB_FREQ_DIFF (15kHz) of channel A
// this should leave at least one frequency

	{
		...
		for (i = 0; i < NUM_FCLR_FREQ; i++)
		{
//			printf(": %d ", lowest_freqB[i]);

			if (abs(lowest_freqB[i] - *freqA) < AB_FREQ_DIFF)
			{
//				puts("*");
				lowest_freqB[i] = -1;
				nf--;
...
//		puts("");
	 ...
	 // find the first valid frequency

		for (i = 0; i < NUM_FCLR_FREQ; i++)
		{
			if (lowest_freqB[i] != -1) break;
		}
	...
	// fill the invalid frequencies with the best valid frequency
		...
		{
			if (lowest_freqB[j] == -1) lowest_freqB[j] = lowest_freqB[i];
