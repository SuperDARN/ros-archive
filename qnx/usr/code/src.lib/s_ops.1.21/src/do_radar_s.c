...
/*
 $Log: do_radar_s.c,v $
 Revision 1.10  2002/03/26 16:55:05  barnes
 Fixed redundant headers.

 Revision 1.9  2002/03/26 16:45:45  barnes
 Removed redundant header.

 Revision 1.8  2002/02/26 14:27:30  barnes
 Fixed bug detected by Paul Hamadyk concerning setting of gain.

 Revision 1.7  2001/10/15 16:26:01  barnes
...
int do_radar_s(int intt,int *delay,clock_t tick,int debug,
  ...
  *nave_B=0;
  ...
  memset(pwr0_A,0,MAX_RANGE*sizeof(float));
  memset(acfd_A,0,MAX_RANGE*2*LAG_TAB_LEN*sizeof(float));

  memset(pwr0_B,0,MAX_RANGE*sizeof(float));
  memset(acfd_B,0,MAX_RANGE*2*LAG_TAB_LEN*sizeof(float));

  if (xcfd_A !=NULL) memset(xcfd_A,0,MAX_RANGE*2*LAG_TAB_LEN*sizeof(float));
  if (xcfd_B !=NULL) memset(xcfd_B,0,MAX_RANGE*2*LAG_TAB_LEN*sizeof(float));
