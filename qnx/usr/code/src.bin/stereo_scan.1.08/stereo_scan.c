...
char program_name[40]={"stereo_scan"};
char program_id[80]={"$Id: stereo_scan.c,v 1.8 2000/11/29 17:16:37 barnes Exp $"};
char command[1024];
...
void main(int argc,char *argv[]) {
  ...
  tfreq_A=13500; /* just a dummy frequency */
  tfreq_B=14500; 
  ...
  strncpy(combf_A,program_id,80);   
  strncpy(combf_B,program_id,80);   
  ...
  if ((ad_id_s[CHN_A]==-1) || (ad_id_s[CHN_B]==-1) || (dio_id_s==-1)) {
  ...
  do {
   ...
   /* test and re-open files if necessary */
    ...
    for (bmnum_A=0;bmnum_A<16;bmnum_A++) {
      ...
      fclr_s(debug,start_freq_A,start_freq_A+freq_range_A,5,
