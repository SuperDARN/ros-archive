...
char cmdlne[1024];
char progid[80]={"$Id: normalsound.c,v 2.3 2006/02/07 20:55:52 barnes Exp $"};
char progname[256];
...
int find_optimal_freq(int start_freq, int cutlass, int fw, int sounder_freqs[], int sft, struct sounder_struct *sounder_data, int act_snd_rec)
{
  ...
  /* set the optimal freq to start_freq in case we don't find a good one */
  def_freq=start_freq;
  if( !cutlass )
    for ( def_freq_bin= 0; def_freq_bin < sft; def_freq_bin++) {
      if ( (start_freq >= sounder_freqs[ def_freq_bin]) && (start_freq <= sounder_freqs[ def_freq_bin] + frqrng) )
        break;
    }
/*
  else
