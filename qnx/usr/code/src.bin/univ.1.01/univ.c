...
/* 
 $Log: univ.c,v $
 Revision 1.1  2001/03/09 17:48:27  barnes
 Initial revision
...
char program_name[40]={"univ"};
char program_id[80]={"$Id: univ.c,v 1.1 2001/03/09 17:48:27 barnes Exp $"};
char command[1024];
...
int check_frang(int first_range,int range_gate){
  int i;
  i=(first_range/range_gate)*range_gate;
  return(i);
}
...
void main(int argc,char *argv[]) {
  ...
  option_add(&cmdlne,"sb",'i',&special_beam);
  option_add(&cmdlne,"sbs",'i',&sbs);
  option_add(&cmdlne,"st",'i',&scan_period);
  option_add(&cmdlne,"alt",'x',&alt);
  option_add(&cmdlne,"id",'i',&subid);
 ...
 /* setup the remaining shell variables */
  ...
  radar_shell_parse(&rstable,                                      
                    "sbm l ebm l \                           
                     ...
                     night_mpinc l freq_range l xcf l special_beam l\
                     sbs l scan_period l alt l id l",                        
                     &sbm,&ebm,                              
                     &day_start_freq,&night_start_freq,                  
                     &day_frang,&night_frang,                            
                     &day_mpinc,&night_mpinc,                            
                     &freq_range,
                     &xcf,&special_beam,&sbs,&scan_period,&alt,&subid);        
 ...
 /* setup the comment block */
  ...
  sprintf(cbf_str,"univ ID=%04d.%04d",cp,subid);
  strcpy(combf,cbf_str);
  strcpy(program_name,cbf_str);
  ...
  do {
    ...
    if (test_reopen(2,0,0) !=0) {
     ...
     if ((scan_period>=0) && (special_beam==-1))
    ...
    do {
     ...
     if (day_time()==1) {
      ...
      if (special_beam !=-1) sbss--;

      if ((special_beam !=-1) && (sbss==0)) {
        sbss=sbs;
        old_beam=bmnum;
	bmnum=special_beam;
	scan=-32768;
    ...
    if ((exit_poll==0) && (scan_period>=0)) wait_boundary(scan_period);
